...
define :php_fpm_pool, :template => 'php-fpm-5.5-pool.conf.erb', :enable => true do

  pool_name = params[:name]

  conf_file = "#{node['cog_newrelic']['php']['pool_conf_dir']}/#{pool_name}.conf"

  if params[:enable]
    template conf_file do
      only_if   "test -d #{node['cog_newrelic']['php']['pool_conf_dir']} || mkdir -p #{node['cog_newrelic']['php']['pool_conf_dir']}"
